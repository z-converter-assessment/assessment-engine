...
@router.get("/")
async def list_servers(request: Request, service: ServerService = Depends(get_service)):
    servers = await service.list_servers()
    return templates.TemplateResponse(request=request, name="servers/list.html", context={"servers": servers})


@router.get("/{server_id}")
async def get_server(server_id: uuid.UUID, request: Request, service: ServerService = Depends(get_service)):
...
async def get_history(server_id: uuid.UUID, request: Request, service: ServerService = Depends(get_service)):
