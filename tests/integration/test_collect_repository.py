...
async def test_upsert_server_same_host_id_converges_to_single_row(
    collect_repo: CollectRepository,
    db_session,
):
    """host_id 단일 UNIQUE 키 — 같은 host_id 면 hostname 이 달라도 한 row 로 수렴 (ADR 0022).

    host_id 는 primary MAC + /etc/machine-id 합성 SHA-256 이라 호스트마다 고유.
    hostname 은 display(non-UNIQUE)라 같은 host_id 로 다른 hostname 이 와도 동일 row 의
    ...
    hostname = (
        await db_session.execute(
            text("SELECT hostname FROM server_inventory WHERE host_id = :m"),
            {"m": "mid-dup"},
        )
    ).scalar_one()
...
async def test_find_server_id_distinct_host_ids_isolated(
    collect_repo: CollectRepository,
):
    """서로 다른 host_id 는 각각 다른 server_id 로 격리 (host_id 단일 UNIQUE 매칭)."""
    sid_a = await collect_repo.upsert_server(make_inventory(host_id="mid-iso-a", hostname="a"))
    sid_b = await collect_repo.upsert_server(make_inventory(host_id="mid-iso-b", hostname="b"))
