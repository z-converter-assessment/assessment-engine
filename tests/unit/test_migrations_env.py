...
def test_database_url_with_slash_and_plus_password_survives_set_main_option():
    ...
    settings = WebSettings(postgres_password="Jp/MmSP9QM6gizrtHEYTKP0gFHi3pr1BGworLrjBqt4=")  # type: ignore[arg-type]
...
def test_database_url_without_percent_char_unaffected():
    ...
    settings = WebSettings(postgres_password="plainpassword123")  # type: ignore[arg-type]
    url = settings.database_url
    assert "%" not in url
