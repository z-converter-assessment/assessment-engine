"""environment_report_mapper — 환경 보고서 합성 + P3 회피 precompute 필드 검증.
...
from assessment_engine.web.services import environment_report_mapper as erm
...
def test_provisioning_segment_defs_single_truth():
    """environment_report_mapper._PROVISIONING_SEGMENT_DEFS 는 mappers._DONUT_SEGMENT_DEFS alias."""
    assert erm._PROVISIONING_SEGMENT_DEFS is m._DONUT_SEGMENT_DEFS


def test_capacity_imminent_days_single_truth():
    """environment_report_mapper._CAPACITY_IMMINENT_DAYS 는 mappers._CAPACITY_IMMINENT_DAYS alias."""
    assert erm._CAPACITY_IMMINENT_DAYS == m._CAPACITY_IMMINENT_DAYS
...
def _make_row(public_id: str, hostname: str, rec: str = "optimal") -> ReportRowItem:
    ...
    return ReportRowItem(
        server_id=1, public_id=public_id, hostname=hostname,
        role="other", os_display="ubuntu 22.04", kernel_version="6.5.0",
        internal_ip="10.0.0.1", is_online=True,
        cpu_avg_pct=10.0, cpu_p95_pct=20.0, cpu_peak_pct=30.0,
        mem_avg_pct=20.0, mem_p95_pct=30.0, mem_peak_pct=40.0,
        load_15m_max=None, swap_used=False,
        recommendation=rec, recommendation_label=rec, badge_class=f"rec-{rec}",
        risk_level="normal", risk_label="정상", risk_badge_class="rec-optimal",
    )
...
def test_to_environment_report_precomputes_count_fields():
    ...
    overview = EnvironmentOverview(
        ...
        total_vcpus=12, total_memory_gb=48.0, total_disk_gb=300,
        utilization=[], util_sample_size=3,
        risk_donut=[], risk_donut_total=0, risk_high_count=0,
    )
    from assessment_engine.web.view_models import ReportTotals
    base = ReportSummary(
        rows=rows, period_days=14, total=3, online=3,
        risk_attention=0, risk_high=1,
        totals=ReportTotals(total_vcpus=12, total_memory_gb=48, total_disk_gb=300),
        summary_bullets=[], role_distribution={"other": 3},
    )
    attention = AttentionSignals(gap_warnings=[])  # 모든 신호 빈 list

    result = to_environment_report(
        view="customer",
        base=base, overview=overview, attention=attention,
        details=[], time_range="14d",
...
def test_to_environment_report_classification_dist_empty_rows_zero_pct():
    ...
    overview = EnvironmentOverview(
        ...
        utilization=[], util_sample_size=0,
        risk_donut=[], risk_donut_total=0, risk_high_count=0,
    )
    from assessment_engine.web.view_models import ReportTotals
    base = ReportSummary(
        rows=[], period_days=14, total=0, online=0,
        risk_attention=0, risk_high=0,
        totals=ReportTotals(0, 0, 0),
        summary_bullets=[], role_distribution={},
    )
    result = to_environment_report(
        view="customer",
        base=base, overview=overview, attention=AttentionSignals(gap_warnings=[]),
        details=[], time_range="14d",
        anchor_at=datetime(2026, 5, 12, tzinfo=UTC),
        generated_at=datetime(2026, 5, 12, tzinfo=UTC),
    )
