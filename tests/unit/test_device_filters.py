...
@pytest.mark.parametrize("name, expected", [
    ("sda", True), ("sdb", True), ("sdaa", True),
    ("vda", True), ("hda", True), ("xvda", True),
    ("nvme0n1", True), ("mmcblk0", True),
    # 파티션은 physical 아님
    ("sda1", False), ("nvme0n1p1", False),
    # LVM은 physical 아님
    ("dm-0", False), ("md0", False),
    # 가상·기타
    ("loop0", False), ("", False), ("foo", False),
])
...
@pytest.mark.parametrize("name, expected", [
    ("dm-0", True), ("dm-12", True), ("md0", True), ("md127", True),
    ("sda", False), ("nvme0n1", False),
])
...
@pytest.mark.parametrize("name, expected", [
    ("sda1", True), ("vda12", True), ("nvme0n1p1", True), ("mmcblk0p1", True),
    ("sda", False), ("nvme0n1", False), ("dm-0", False),
])
...
@pytest.mark.parametrize("fstype, mount, expected", [
    ("proc", "/proc", True),
    ("sysfs", "/sys", True),
...
@pytest.mark.parametrize("mount_major, mount_minor, expected_name", [
    (8, 0, "sda"),     # 디스크 자체
    (8, 1, "sda"),     # sda의 파티션
