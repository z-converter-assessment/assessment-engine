...
@pytest.mark.parametrize("pct, severity", [
    (None, "ok"),
    (0.0, "ok"),
    (50.0, "ok"),
    (74.9, "ok"),
...
def _summary(**overrides) -> ServerSummary:
    base = dict(
        id=1, public_id="pub-1", machine_id="m-1", hostname="host",
        os_id="ubuntu", os_version="22.04",
        cpu_cores=4, mem_total_kb=8 * 1024**2,
        ip_external=None,
...
def test_to_storage_detail_filters_virtual_mounts():
    storage = StorageWithUsage(
        server_id=1, public_id="pub-1", hostname="h",
