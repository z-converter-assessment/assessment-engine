...
- right-sizing 분류 단일 진실 = `recommendation.assess(stats) -> Assessment(recommendation, triggers, unmeasured)` (evidence 기반, OS-aware, ADR 0029 정정). `classify` 는 분류 enum 만 돌려주는 호환 wrapper. 판정 순서 = under(위험 신호 OR) -> idle -> shutdown -> insufficient_data -> over -> optimal — under 가 idle/shutdown 보다 우선이다(saturation·압박 신호가 "미사용" 분류를 가로채지 않음, 예: CPU idle 인데 swap 발생 = under). 합성 규칙: under = 위험 신호 OR(하나라도 hit 되면 발화, 누락 0) / over = cpu/mem 둘 다 다운사이즈 임계 이하일 때만(보수적) / insufficient_data = utilization(cpu/mem) 둘 다 부재 + under 신호도 없을 때만(후순위 — swap 등 saturation 신호가 있으면 util 부재여도 under 로 결론). hit 신호(triggers)를 근거로 동반 — report mapper 권고(`_build_under_provisioned_reason`)와 attention 자원 부족 카드(`to_capacity_warning_item` — 발화 원인 `active_causes` os-neutral 라벨 `_CAUSE_LABEL_BY_TRIGGER`, 환경 요약 원인 집계 단일 소스)가 `assess.triggers` 재사용(임계 재계산 금지), stats 생성은 `build_resource_stats` 공용 — report·attention·서버목록(`to_server_list_item`)·도넛(`_assemble_overview`)이 동일 입력(net baseline·worst_mount disk 포함, 화면 간 idle/shutdown 정합 의무). saturation 3축은 모두 os-aware helper 단일 진실 경유 의무(임계 재계산·직접 해석 금지): `cpu_saturated`(Linux procs_running p95/cores>=1.0 / Windows Processor Queue Length p95/cores>=2), `mem_saturated`(Linux swap page-out=`swap_saturation` / Windows Memory Pages Input/sec(하드폴트, mmap 미혼입) rate p95>=20; Windows pagefile 사용량은 상시 baseline 이라 하드폴트율로 판정, `if raw.swap_used` 직접 해석 금지), `disk_io_saturated`(Linux await_p95>20ms / Windows Avg Disk Queue Length `disk_queue_p95`>=2; await 는 디바이스 지연 직접 신호 — iowait 는 게스트 스케줄링 왜곡·다중코어 희석이라 미사용). Windows 는 세 축 모두 실측(agent 가 `saturation.{cpu_run_queue,disk_queue}` + `mem_pages_input`(하드폴트) 발행) — 해당 perflib 못 읽거나 미부착(예: virtio 에 diskperf 미부착 -> disk queue 빈 배열)인 축만 helper 가 None 반환 -> `unmeasured` 기록 -> `is_partial`(=bool(unmeasured)) -> `ReportRowItem.is_partial` 로 "포화 수치 미관측" confidence 노출 (분류는 utilization/capacity/측정된 나머지 포화 축으로 완결, "데이터 부족" 아님 — cpu_p95/mem_p95 산출되는 한). cpu_stat 은 Windows nice/iowait/irq/softirq/steal 이 null 이라 cpu_total 을 성분 COALESCE 합으로 정규화(cagg·`compute_cpu` 동일, Windows=user+system+idle). 분류 명세·임계 근거(USE Method·AWS/Azure/GCP advisor·MS 출처)·한계 단일 진실 = `docs/architecture/right-sizing.md`.
...
| 분류 신호·OS 분기 (USE Method 축·임계·trigger, ADR 0029 evidence) | (1) `recommendation.py` `assess`/`Assessment`(triggers·unmeasured)·임계 상수·`swap_saturation`·`cpu_saturated`·`mem_saturated`·`disk_io_saturated`(os-aware) helper·`ResourceStats` 필드(`disk_queue_p95`·`cpu_run_queue_p95`·`mem_pages_input_rate_p95` 등) (2) saturation 원자료면 `report_aggregate` SQL(cagg 집계 컬럼)·`ReportRowRaw`·`build_resource_stats` 배선 동시 (3) trigger 키 추가 시 report 권고(`_TRIGGER_ACTION_KO`)·`_build_diagnosis`·attention 자원 부족 카드 원인 라벨(`_CAUSE_LABEL_BY_TRIGGER`+`_UNDER_CAUSE_ORDER`)·지표 os-aware·single_report `_build_saturation_axes` 동시 갱신 (4) stats 생성은 `build_resource_stats` 공용(report·attention·서버목록·도넛 단일 진실) — 직접 해석·임계 재계산 금지 (5) 표시 N/A·confidence(`is_partial`=bool(unmeasured)) 마커 (ViewModel precompute + 템플릿) (6) `docs/architecture/right-sizing.md`(명세·근거 단일 진실) + `docs/architecture/web/services.md` "OS 분기" + `right_sizing_thresholds.html`·`_thresholds_reference.html` + ADR 0029 정정 |
