...
- right-sizing 분류 단일 진실 = `recommendation.assess(stats) -> Assessment(recommendation, triggers, unmeasured)` (evidence 기반·os-aware). `classify` 는 분류 enum wrapper.
...
- triggers·stats 재사용 의무 — report 권고(`_build_under_provisioned_reason`)·attention 자원 부족 카드·서버목록·도넛이 `assess.triggers` + `build_resource_stats` 공용 입력을 쓴다 (화면 간 분류 정합, 임계 재계산 0).
