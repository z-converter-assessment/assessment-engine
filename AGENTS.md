...
프로젝트 규약 정본은 이 파일이다. 요청이나 변경 유형에 맞는 `.agents/skills/*/SKILL.md` 또는 `.agents/reviewers/*.md`가 있으면 해당 파일을 먼저 읽고 적용한다. 특정 에이전트의 명령어, 플러그인, 심링크에 의존하지 않는다.
...
- 카운터 메트릭(CPU jiffies/disk/net bytes) 집계는 continuous aggregate + timescaledb_toolkit `counter_agg` 사전집계 단일 진실. counter reset(재부팅/agent재시작/wraparound)은 `counter_agg` 가 값-감소 기준 일률 처리 — 보고서 집계(`get_report_aggregate`/`get_report_*_baseline`/`get_report_cpu_breakdown`)에서 hand-rolled LAG + boot_time gate 부활 금지. 차트(`get_metric_trend`, 동적 버킷)는 목적상 raw 유지.
...
- 시계열/시간 흐름 endpoint(`get_metric_snapshots` / `GET /api/tasks` 등): cursor 기반 — `cursor: datetime | None` + `limit`. 시간 역순 스크롤. page 번호 의미 없음 (계속 새 데이터 들어옴).
...
서비스 카테고리 분류는 도메인 모듈 `assessment_engine/domain/service_classifier.py` 소관 — web/consumer 공용이고 web 역의존 0(`right_sizing.py` 동급). 템플릿은 `service_badge_class` 필터로 category -> CSS 클래스 변환만 한다(P3). 메커니즘 상세 = `docs/reference/web/services.md`.
...
원칙: 자동화 변환(sed, 일괄 문자열 치환, 디렉토리 mv, Python 일괄 갱신) 직후 검증을 2 채널로 분담. 로컬 훅은 두지 않는다 — 우회 가능한 자리라 강제 수단이 못 된다(`docs/guides/conventions.md` 5절).
- 메인 세션 — 자가 검증. 변환 직후 매 회 의무 (아래 4 항목).
- 독립 감사 (`.agents/reviewers/code-reviewer.md` / `schema-contract-auditor.md`) — 본 절 맥락(변환 직후 점검)에서는 사용자가 코드 리뷰나 스키마 일관성 확인을 명시했을 때만 발동. PR 게이트의 코드 리뷰는 별개 채널이며 배치는 `docs/guides/pre-pr-checklist.md` 0절이 정한다.
...
- 시그널 로그(`_log_time_invariants`/`_track_agent_restart`)는 쿨다운/빈도 카운터 의무 — 매 메시지 발생 시 진짜 시그널 매몰.
...
변경 유형별 동시 갱신 위치 표(시계열/inventory 컬럼/routing key/페이로드 schema/분류 임계/환경변수/ViewModel 파생 필드/JSON API/보고서 스냅샷/조건부 UI/외부 의존/차트 MetricType/비동기 보고서/install task lifecycle) = `.agents/skills/change-impact/SKILL.md` 단일 진실. 해당 유형 변경 시 본 스킬 로드 의무.
...
- 기능 구현 시 새 주석은 외부 계약, 비자명한 제약, 의도적인 예외에만 추가한다. 코드 흐름 설명, 변수 값 재서술, 구현 순서 설명 주석은 작성하지 않는다. 수정한 코드 주변의 불필요한 기존 주석은 함께 제거한다.
