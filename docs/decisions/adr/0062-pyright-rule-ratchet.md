...
릴리즈 단위 문서 감사에서 앞선 블록들의 수치 슬립 셋이 잡혔다. 본문은 고치지 않고 여기에 바로잡는다.

src 의 strict 위반은 2907건이다 (Context 절 수치가 맞다). "2903건 중 2903건" 은 선언 채움 시점의 재측정값을 전량으로 적은 것이고, 그 시점에 소진한 것은 선언 관련 규칙이며 값 추론 Unknown 423건은 다음 단계에서 소진했다.

tests 잔량은 2060건이다. "1906건" 은 tests 를 include 에 넣기 전 측정값이고, 편입 과정에서 픽스처·parametrize 어노테이션이 드러나 실측이 올라갔다.

`executionEnvironments` 블록은 최종적으로 둘이었다 (`scripts`·`src`). 앞 블록의 "블록을 지운다" 단수 표기는 그 시점에 하나만 있었기 때문이고, 최종 제거 대상은 둘이다.
