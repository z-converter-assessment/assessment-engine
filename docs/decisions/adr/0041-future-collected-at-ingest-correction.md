...
포기한 것 / 한계
- `collected_at` 은 시계열 자연키(#C1 `UNIQUE(server_id, collected_at)`). 수신시각 기준 보정은 비결정적(재처리 시 now 상이)이라 D2 2단(DB UNIQUE)을 약화한다. 단 D2 1단(message_id SET NX, 24h)이 재전송을 DB 도달 전에 흡수하므로 실노출은 "Redis 장애 + 재전송 + 시계불량" 동시 발생의 좁은 창뿐. 상세·재검토 트리거는 `docs/explanation/tradeoffs.md` T17.
