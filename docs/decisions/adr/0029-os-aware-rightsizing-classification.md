...
> - 명세·임계·OS 분기·한계 단일 진실 문서 `docs/reference/right-sizing.md` 신설 (CLAUDE.md 가 참조하던 공백을 메움). `_thresholds_reference.html` 판정 순서 표도 under=1 로 동기화.
...
> - 남은 한계는 축 부재가 아니라 (a) Windows 메모리 페이징 절대 임계(1000 pages/sec)의 근거 취약(잠정·튜닝 대상) (b) perflib 미발행 축의 미관측 — `docs/explanation/tradeoffs.md` T14 갱신.

## Context

ADR 0027 로 Windows agent 가 합류했다. agent 는 raw 값을 canonical(Linux `/proc` 모델)로 변환 발행하고 엔진은 OS 무관 단일 공식으로 계산한다(`docs/reference/contracts/agent-data.md`). 그러나 right-sizing 분류(`recommendation.classify`)는 USE Method 임계를 OS 무관(OS-blind)으로 적용해 왔고, 이게 Windows 에서 두 가지 왜곡을 낳았다:
...
단점·한계
- Windows 는 saturation 축(load/iowait)을 못 봐 utilization 축만으로 판정 — saturation 병목을 분류로 못 잡는다 (부분 평가, `docs/explanation/tradeoffs.md` T14).
...
- `docs/reference/web/services.md` "OS 분기" 절 — 분류 deep-dive
- `src/assessment_engine/web/templates/reports/right_sizing_thresholds.html` — 판정 순서·OS 분기 UI 참고자료
- `docs/explanation/tradeoffs.md` T14 — Windows 부분 평가 한계
