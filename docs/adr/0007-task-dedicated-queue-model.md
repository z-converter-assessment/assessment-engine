...
- 인프라: `infra/agent.env` + `infra/agent.env.example` + `dev-up.sh` REQUIRED_AGENT_KEYS 및 heredoc.
- 문서: `docs/architecture/agent.md` (메시지 데이터 형식) + `docs/architecture/rabbitmq.md` (토폴로지 표 + 권한 모델) + `docs/adr/0002-*.md` (Superseded 표기) + 본 ADR.

## 관련 문서

- `docs/architecture/agent.md` — 메시지 데이터 형식 단일 진실
- `docs/architecture/rabbitmq.md` — 토폴로지 / 큐 정책 / dev·prod 분기
