...
느린 여덟은 hypothesis property-based 테스트다. 입력을 생성해 여러 번 돌리는 방식이라 개당 8~24초가 걸리는 것 자체는 정상이다.

로컬 120초가 CI 376초가 되는 차이는 둘이다 — 러너 성능, 그리고 커버리지 측정.

CI 명령은 이렇다.

```
uv run pytest tests/unit/ -v --cov=assessment_engine --cov-report=term
```

`--cov=assessment_engine` 은 테스트가 도는 동안 그 패키지의 어느 줄이 실행됐는지 세라는 뜻이다. 인터프리터가 줄을 밟을 때마다 훅이 걸려 기록하므로 이 추적이 테스트를 느리게 만든다. `--cov-report=term` 은 그 결과를 표로 찍는다 — "이 파일 412줄 중 31줄 미실행, 92%" 같은 것.

없는 옵션이 하나 있다. `--cov-fail-under=N` — 커버리지가 기준 미만이면 CI 를 실패시킨다.

그래서 지금은 3~4분을 들여 측정하고, 표를 GitHub Actions 로그에 찍고, 숫자가 몇이든 통과한다. 측정 비용은 매 PR 자동으로 나가는데 산출물은 사람이 로그를 펼쳐야만 소비된다. develop PR 마다 그걸 펼쳐 보는 사람은 없다.

커버리지 자체가 무의미한 것이 아니다 — 표를 보면 어느 모듈에 검증 공백이 있는지 알 수 있다. 비용과 소비가 어긋나 있는 것이 문제다. 갈 길은 둘이다. 임계값을 걸어 CI 가 대신 판정하게 해서 3~4분이 실제로 무언가를 막게 하거나, 자동 실행에서 빼고 필요할 때만 돌려 3~4분을 아끼거나.
...
2. 커버리지를 develop PR 에서 뺀다 (위 "갈 길 둘" 중 후자). 게이트로 쓰기로 하면 반대로 `--cov-fail-under` 를 붙이는 쪽이다 — 그때는 시간이 아니라 기준치를 정하는 문제가 된다.
