...
- 22페이지 환경과 서버 현황 조회: 환경 요약과 워크로드 및 자원 적정성 카드, 개별 서버의 시스템 정보와 실시간 메트릭 카드를 완결된 영역 단위로 배치. 환경 범위와 서버 범위에서 제공하는 정보의 차이를 비교
...
- 26페이지 보고서 유형과 용도: 환경과 개별 서버 범위, 고객과 엔지니어 독자의 2 x 2 조합을 원본 화면의 완결된 카드 4개로 비교. 엔지니어용은 같은 평가 결과를 바탕으로 상세 메트릭, 시스템 구성과 판정 근거를 추가하는 관계
