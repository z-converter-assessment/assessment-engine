...
4. `decisions/`(adr·rfc)는 이력 아카이브지 의존 대상이 아니다. 라이브 문서(reference·guides·explanation·AGENTS.md)는 결정 번호를 전제로 참조하지 않는다 — 필요한 사실은 라이브 문서 안에 인라인으로 있어야 한다.
...
| `AGENTS.md` | AI 에이전트 운영 규칙 — 불변식·금지·지도, 안 깨려면 알아야 할 것만 | 얇게·매 세션 로드 |
| `.agents/` | 작업 스킬과 읽기 전용 감사 프롬프트 | 에이전트 공용 자원 |
...
AGENTS.md는 결정·금지를, reference는 구현 방식을 담는다 — 둘이 같은 것을 두 번 쓰지 않는다. 문서는 명령을 복제하지 않고 왜 그 명령을 쓰는지와 옵션·변형·맥락만 담는다.
