...
| 본 repo dev 작업·코드 규약 (Docker·dependencies·pipeline 검증·testing·conventions) | `development/` |
...
│   └── observability.md   로그 레벨·외부 의존 실패 매트릭스·Prometheus `/metrics`·LOG_FORMAT toggle + 향후 확장 (Request ID 분산 trace)
