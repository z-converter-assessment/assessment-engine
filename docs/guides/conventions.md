...
검사·관용구·강제 지점의 단일 진실이다. 정책의 근거는 AGENTS.md #F1·#F5 가 갖고, 여기서는 무엇을 어떻게
지키는지를 적는다.
...
심볼을 패키지 안 형제 모듈이 쓰는 관용구를 양쪽에서 때리는 것들이다 — 한쪽은 그 사용을, 다른 쪽은 반대편에서
"안 쓰인다"고 잡는다. 다른 하나는 프리셋이 끄지만 위반이 0 이라 켜 두는 규칙으로, 죽은 코드와 낡은 억제 주석을
잡는다. 목록과 사유는 설정 파일이 갖는다.
...
주석에 적고 제외한다. 제외한 규칙의 목록과 사유는 설정 파일이 갖는다.
...
자동화 변환(sed, 일괄 문자열 치환, 디렉토리 mv, 일괄 스크립트) 직후에는 AGENTS.md #F5 의 4 항목을 매번 돌리고,
변환 유형별로 다음을 더 본다.

| 유형 | 추가 검증 |
|------|---------|
| sed / 일괄 문자열 치환 | 들여쓰기 무관 패턴(`^[[:space:]]*`)으로 다시 훑고, 의도한 스코프 밖(함수 외부/문자열 리터럴 안)에 걸린 곳을 grep 한다. 식별자 일부가 다른 식별자에 포함되는 경우(`_x` -> `x` 가 `test_x` 를 건드리는 등)를 특히 본다 |
...
- `docs/reference/web/type-contract.md` — 프론트 타입 계약 절차
- `docs/guides/ci-setup.md` 3.4 — required check 등록 목록
