...
| 승격 직전 develop PR | 위 전부 + 릴리즈 단위 문서 검증(`origin/main..HEAD`) | 1~5 | `/pr` |
| main PR | 통합 테스트 · 커밋 형식 | 검증만 | `/pr --base main` |
...
릴리즈 단위 문서 검증은 승격 직전 develop PR 이 갖는다. 개별로는 맞는 서술이 릴리즈 단위로 모이면 어긋날 수 있고 그건 묶어 봐야 보이므로, 승격 범위(`origin/main..HEAD`) 전체에 doc-auditor 를 돌린다.
...
- [1.4] 중복 함수·중복 상수 0건. 같은 의미는 단일 모듈 (UI badge 임계 = `mappers/constants.py` / USE Method 임계 = `right_sizing.py`). 두 도메인 혼용 0건 (#E3).
...
- [2.4] 변경·추가한 임계 상수(`mappers/constants.py` · `right_sizing.py` · `_USAGE_DANGER_PCT` 등) 테스트 하드코딩 0건 — 모두 import.
