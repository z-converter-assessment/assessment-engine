...
| 환경 진단 | `POST /api/v1/diagnostics` scope=environment + `GET /diagnostics?ids=<job_id>` | 분류 분포 카운트 + 우선 검토 권장 한 줄 narrative. 즉시 발행 또는 스케줄러 매일 03시 자동. 진단 결과 페이지가 환경 보고서 iframe 2개 (customer·engineer view) 미리 렌더 — 운영자가 한 화면에서 두 view + 진단 narrative 모두 확인 |
