...
본 문서는 사이드바 "모니터링" 그룹 6 페이지 — 환경 개요 홈(`/`)·서버 목록(`/servers`)·환경 자원 평가(`/environment/assessment`)·네트워크 토폴로지(`/environment/topology`)·실시간 현황(`/environment/realtime`)·환경 성능 추이(`/environment/metrics`) — 산출물의 존재 의의·구현 의도·근거를 정리한다. 라우터·서비스·매퍼·정적 자원 deep dive는 `docs/architecture/web/` 별도. 사이드바 네비 정의 단일 진실은 `web/templating/setup.py` `NAV_GROUPS`.
...
L3 subnet 공동소속 추론 그래프 — 인터랙티브 Cytoscape.js (vendored) 렌더. 화면 전용 (보고서는 정적 서브넷 요약 표만, `docs/products/environment-report.md`).
...
- CPU·메모리·디스크·네트워크·디스크 I/O 추이 차트 + CPU 분류 추이.
