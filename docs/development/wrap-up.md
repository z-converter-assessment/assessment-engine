...
- Stage 4 의 grep 검사는 본 명세와 CLAUDE.md 전문(前文) 단일 진실 정의를 제외 — `rg ... --glob '!docs/development/wrap-up.md' --glob '!.claude/CLAUDE.md'` 또는 출력 후 메타 인용 필터링. CLAUDE.md 본문 결정 항목은 검사 대상 유지.
...
목적:
- 본 feature 가 추가·변경한 모든 public 함수·핸들러·라우터에 의미 있는 테스트가 존재하고, 본 repo 테스트 패턴(`docs/development/testing.md`)을 따른다.
...
도구:
- 직접 수행 (레이어 결정 + 패턴 작성). 테스트 정책 단일 진실 `docs/development/testing.md`.
...
- [3.2] OIDC·GHCR 인증 필요한 step(sigstore 서명 · cosign · GHCR push)은 본질적으로 CI 전용 — 로컬 skip (그 직전까지 산출물·파일명 패턴·액션 resolve 는 검증됨).
...
목적:
- 본 feature 가 만진 영역의 영구 문서(`docs/architecture/`·`docs/operations/`·`docs/products/`·`docs/tradeoffs.md`)가 현행 코드를 정확히 반영.
...
- [4.5] `docs/tradeoffs.md` T 항목 중 본 feature 가 해결·악화시킨 것, 새 트레이드오프 — 큐 적재.

중복 없음 (4):
- [4.6] 같은 사실(임계 상수·UNIQUE 키·routing key·환경변수 default·트랜잭션 경계)이 영구 문서 두 곳 이상 정의 0건. 단일 진실 후 나머지는 포인터(`상세는 X 절`).
- [4.7] CLAUDE.md 본문과 `docs/architecture/*` deep dive 책임 혼선 0건. CLAUDE.md = 결정·원칙·금지. deep dive = 동작·흐름·매트릭스·카탈로그.
...
- [4.11] 임시 상태(`TODO`·`FIXME`·`XXX`·"작업 중"·"향후"·"차후") 0건. 검사: `rg -i '\b(TODO|FIXME|XXX|작업중|향후|차후)\b' docs/architecture/ docs/operations/ docs/products/ docs/development/`. 임시는 `docs/temp/` 또는 PR 본문.
...
원칙 (2):
- [4.16] `docs/temp/` 인용 0건 (양방향). 검사: `rg 'docs/temp' src/ docs/architecture/ docs/operations/ docs/products/ docs/development/ docs/adr/ CLAUDE.md`.
- [4.17] 영구 문서 간 같은 사실의 양방향 의존(순환) 0건. 상위(CLAUDE.md) → 하위(`docs/architecture/*`) 단방향. 동등 계층은 다른 책임의 단일 진실 간 cross-reference 만 허용 — 같은 사실을 양쪽에 복제 금지.

도구:
- 직접 수행 (git diff 변경 코드 -> #F9 + 문서 인덱스 매핑 -> 영구 문서 제안·반영). 자동 적용 전 사용자 승인.

산출물:
- `docs/architecture/*` · `docs/operations/*` · `docs/products/*` · `docs/tradeoffs.md`. CLAUDE.md·ADR 후보는 큐만 (실제 변경 Stage 5). 코드 모순 발견 시 Stage 1 재검 트리거.
...
- [5.2] CLAUDE.md 본문 변경 시 영향 deep dive(`docs/architecture/*` · `docs/operations/*`) 동시 갱신. 시그니처·임계·카탈로그 일치.
- [5.3] CLAUDE.md "본 절 결정" 신규 = (원칙 → 결정 → 금지) 구조 + 외부 포인터는 "상세는 `docs/X` 절" 형식만. 본 절 안 deep dive 본문 dump 0건.
- [5.4] ADR 신규 시 `docs/adr/README.md` 인덱스 갱신. 번호는 마지막 + 1 단조 증가 (재사용 0건).
...
- [5.8] 새 외부 의존(HTTP · 외부 큐) 도입 시 F6 "외부 의존 실패 모드 매트릭스"(`docs/operations/observability.md`)에 행 추가 (fail-open/close · timeout · 재시도).
...
산출물:
- `CLAUDE.md` · `docs/adr/*` · `docs/architecture/*` · `docs/tradeoffs.md` · `docs/README.md` · `docs/operations/observability.md`.
