...
- inventory upsert·metrics 저장·server_id 조회 모두 `composite_id` 단일 키 기준 (#C1, ADR 0027)
...
| `get_diagnostic_service(db)` | `DiagnosticService` (ADR 0004 — 보고서 발행·이력 facade) | query_repo + session_factory + `BaseDiagnosticRepository` factory |
...
- `broker_channel`은 lifespan에서 `app.state.broker_channel`에 저장한 영속 channel 재사용 — `TaskService`(install task 발행)가 받아 매 발행마다 connection open/close 안 함 (오버헤드 0). `DiagnosticService`는 보고서를 DB(`diagnostic_jobs`)로 발행·생성(web 내부 job-claim 워커, ADR 0040)이라 broker 미사용
