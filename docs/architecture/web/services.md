...
| `recommendation.py` | USE Method 분류 (idle/shutdown/over/under/optimal). 임계값은 본 모듈 코드가 단일 진실 (모듈 상단 명명 상수) |
...
`/servers/` 첫 페이지에서 두 영역으로 표시. environment_overview는 환경 현황·평균·분포(도넛), attention은 즉시 조치 신호 카드. 시간 축은 F15 단일 윈도우(`recommendation.WINDOW_DAYS=14`).
...
설계 결정:
- risk_top 카드는 2026-05-12 cleanup으로 제거 — 환경 요약 + 프로비저닝 분포 도넛 + capacity_warnings 카드가 정보 흡수.
