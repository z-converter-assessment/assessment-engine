...
본 문서는 RabbitMQ broker 운영의 단일 진실. 코드(`src/assessment_engine/consumer/main.py`)·인프라(`docker-compose.yml`)·계약(에이전트 ↔ 엔진)에 흩어진 broker 관련 결정을 한 곳에 모은다. 토폴로지 코드 동작 관점은 `docs/components/consumer.md`.
...
- `docs/components/consumer.md` — 위 토폴로지를 코드(aio-pika)로 어떻게 declare·subscribe하는지 / 핸들러 / 멱등성 / DB 재시도
- `docs/components/agent.md` — 에이전트 측 publish 동작 / publisher confirm / retry
- `docs/infra/docker.md` — RabbitMQ 컨테이너 정의 / 헬스체크 / 환경변수
- `docs/env.md` — `RABBITMQ_*` 환경변수 키 목록
