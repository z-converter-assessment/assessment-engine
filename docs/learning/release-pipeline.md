...
1절과 2절은 전체 그림을 세우는 오리엔테이션이다. 3·4·6·7절이 L1·L2(OCI 저장 모델과 공개키 서명), 5절과 8절부터 10절이 L3 다. 5절은 빌드 산출물을 세는 절이라 L2 처럼 보이지만 개수를 정하는 것이 워크플로 설정이라 L3 로 둔다.

blob 목록·digest·인증서 실물은 `1.2.1` 릴리즈에서 2026-07-21 에 뽑은 실측값이다. 구조를 실물로 보여주려고 남겨둔 값이며 현재 레지스트리 상태가 아니다.
...
릴리즈의 정의는 `pyproject.toml` 의 version 을 올린 커밋이 main 에 들어오는 것이다. 빌드 소요는 멀티아치 기준 약 4 분이다.
...
[runner VM]   GitHub boots a fresh Ubuntu VM
              read version from pyproject.toml, reject non-stable semver
...
tag: sha256-<index digest>
  |
  v
[6] signature manifest                       <- cosign, one bundle per sign call
```
...
| signature 1 개 | 빌드와 무관한 별도 스텝의 cosign 호출 | 도커 기능이 아님 |

서명 쪽 개수는 빌드가 아니라 cosign 호출 횟수를 따라간다. 태그마다 호출하면 같은 digest 에 번들만 중복 적재되므로, digest 에 한 번만 호출해 alias 개수와 무관하게 1 개로 유지한다.
...
서명 조회 경로가 `sha256-<digest>` 태그인 것은 GHCR 사정이다. OCI 표준에는 referrers API 라는 정식 경로도 있으나 GHCR 이 빈 응답을 주기 때문에 태그 방식이 쓰인다.
...
raw 경로의 형식이 `raw.githubusercontent.com/<owner>/<repo>/<ref>/<path>` 인데, `<ref>` 자리에는 브랜치 이름과 태그 이름과 커밋 해시가 모두 들어갈 수 있다. 태그 이름을 넣으면 그 태그가 가리키는 커밋 시점의 파일 내용이 응답된다. 따라서 그 주소가 유효해지는 시점은 파일을 올리는 시점이 아니라 태그가 생기는 시점, 즉 이미지 발행과 서명이 끝난 뒤 워크플로가 태그를 만드는 순간이다.

태그가 사라지면 주소도 함께 사라진다. 2026-07-21 실측 응답이다.
...
배포는 앞 절들에서 만든 것을 역순으로 소비한다. 방향은 내부망 VM 이 바깥으로 나가 받아오는 쪽이며, 바깥에서 VM 으로 밀어 넣지 않는다. 단계별 절차는 `docs/guides/deploy.md` 3절이 정본이고, 여기서는 순서를 강제하는 인과 둘만 본다.

서명 검증이 pull 앞에 온다. 7 절에서 본 대로 digest 대조는 받은 뒤에야 할 수 있고 그것만으로는 출처를 답하지 못하므로, 바이트를 내려받기 전에 신원을 판정해야 의미가 있다. 검증이 실패하면 아무것도 받지 않는다.

compose 를 이미지와 같은 태그 ref 에서 받는다. 8 절의 두 재료가 서로 다른 출처에서 오는데, 태그 하나로 묶어야 그 이미지가 전제한 토폴로지로 기동한다는 보장이 선다.

## 10. git 태그가 파이프라인의 출력인 이유

태그는 장식이 아니라 파이프라인이 읽고 쓰는 상태다. 그리고 태그를 입력으로 둘지 출력으로 둘지가 실패 처리를 가른다.

태그를 입력으로 쓰면 — 운영자가 `v*` 를 push 하는 것이 트리거가 되고 빌드가 태그에서 버전을 derive 하는 구성이다 — 태그가 먼저 생기므로 발행이 실패해도 태그가 남는다. 재시도하려면 태그를 지우거나 버전을 건너뛰어야 한다.

출력으로 두면 발행과 서명이 모두 성공한 뒤에야 태그가 생긴다. 앞 단계가 실패하면 태그가 없으므로 같은 커밋을 그대로 다시 돌릴 수 있고, 이미 태그가 있으면 중복 발행으로 보아 건너뛴다. 이 저장소가 고른 쪽이며 트리거·버전 출처는 `docs/guides/release.md` 2절이 정본이다.

어느 방식이든 태그의 역할 하나는 공통이다. 배포 스크립트가 그 태그의 raw 경로에서 compose 파일을 받으므로, 태그는 이미지 버전과 compose 토폴로지 버전을 같은 ref 로 묶는 축이다.
