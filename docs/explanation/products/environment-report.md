...
| 분류 분포 | 자원 적정성 5분류 카운트 막대 (한국어 분류명 RECOMMENDATION_LABEL_KO, 영어 enum 미노출) | `rollup_host().recommendation` 결과 카운트 |
...
5분류·트리거 조건 상세는 `docs/reference/right-sizing.md` 4절, 운영자 카탈로그는 `right_sizing_thresholds.html`. 호스트 권고 판정 순서는 같은 문서 3절(`rollup_host().recommendation`).
