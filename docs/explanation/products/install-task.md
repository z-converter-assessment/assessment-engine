...
본 문서는 Install task 산출물(엔진이 원격 호스트에 도구 설치 명령을 발행하는 워크플로)의 존재 의의·구현 의도·근거를 정리한다. 메시지 schema·실행 흐름·기술 세부는 `docs/reference/contracts/agent-data.md` "task.install" / "task.result" 절 + `docs/reference/rabbitmq.md` 별도. Task 별도 큐 모델.
...
발행된 task는 `Task` row에 영속 — `status` (success/failure/pending) + `exit_code` + `duration_ms` + `stdout_tail` + `stderr_tail` + `failure_reason` 6 컬럼 UPDATE. list "ZDM Install" column에 badge로 노출. 클릭 시 modal에서 stdout/stderr 마지막 4 KB까지 확인 가능 — 실패 디버깅 즉시.
...
- `docs/reference/contracts/agent-data.md` "task.install" / "task.result" 절 — 메시지 schema·필드 카탈로그
