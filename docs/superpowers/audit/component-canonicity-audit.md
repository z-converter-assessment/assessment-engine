...
- B3 채택·구현·검증(70대 재실측으로 flip): 2서버 실측으론 "유예"였으나(env_util 4ms 무해), 70대 엔지니어 보고서 발행 시나리오 재실측에서 env_util mem per-ts p95 raw 475.8ms vs cagg 97.6ms(약 5배)로 실질 병목 확인 — 보고서 발행마다 fires, fleet 선형 증가. server_metrics_5m 재생성 마이그레이션(f6f365fd114c, mem byte gauge available/limit + cached/buffered pct 추가, autocommit_block refresh, refresh->policy 순서로 락 충돌 회피) + env_util·memory_breakdown cagg 재배선. up/down 라운드트립(데이터 有) robust + 통합 테스트 18개 PASS(capacity-weighting 보존 test_environment_utilization_capacity_weighted 포함). 출력 DTO 불변이라 mapper/template 연쇄 0. disk env_util 스칼라는 더 가벼워 mem-first 범위 밖(후속).

결론: 실측이 개선 후보 4건을 갈랐다 — 채택 3건(B1 성능·B3 성능·B4 보안, 전부 검증 완료), 기각 1건(B2, 플래너가 이미 최적). B2는 EXPLAIN-선행이 blind churn을 막았고, B3는 반대로 "현실적 규모(70대 보고서)로 재실측"이 성급한 유예를 뒤집었다 — 2서버 테스트가 병목을 245배 과소평가. 교훈: 측정하되 현실적 규모로.
