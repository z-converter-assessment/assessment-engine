...
- Memory: 목표 = ceil(현재총량 x near_peak / 목표%). near_peak = 5분 버킷 max 의 p99.9(관측 피크 대표) — 메모리는 비탄력·OOM 위험이라 평균/p95 아닌 피크 통계로 역산한다. 포화(swap/oom)인데 near-peak 관측이 없으면 현재총량 x 1.30(30% 상향).
...
통계는 14일 창으로 잡는다: 분류·CPU 사이징은 p95, 메모리 사이징은 near-peak(버킷 max p99.9). max·avg 단독은
...
| Memory 사이징 통계·목표 | near-peak(버킷 max p99.9) 80% 착지 | AWS 기본 + 비탄력 피크 대표 |
