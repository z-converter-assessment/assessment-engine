...
| 파일 | 역할 |
|------|------|
| `package.json` | pnpm(packageManager 핀). devDep 은 검사 도구(typescript·openapi-typescript·chart.js·@types/cytoscape) + 화면 캡처용 playwright(`pnpm run screenshot` — 코딩 에이전트 검증용, 사람 워크플로 진입점 없음). 빌드/번들/런타임 산출물 없음. |
