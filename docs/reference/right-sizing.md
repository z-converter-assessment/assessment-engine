...
> 표시 배지 = `rollup_host(stats).recommendation`.
...
분류다. 자원 5개(CPU, 메모리, 디스크 용량, 디스크 I/O, 네트워크)를 USE 평가 3축(이용률, 포화, 오류)으로 판정한 뒤,
...
- 디스크 용량: filling(소진 runway 임박 OR 정적 가드 used% 초과, 바이트·inode 각 축) / capacity_ok. under/over 아닌 "남은 시간" 예측(누적 자원)이고, runway 는 마운트별 이력 전체의 2점(first/last) fill rate 외삽이다. 바이트 소진은 확장 목표 GB 를 동반하고, inode 소진은 파일 정리 또는 파일시스템 재구성 조치다.
- 디스크 I/O: io_bound(응답 지연 await p95 임계 초과) / io_ok. 증분 불가라 사이징이 없다 — 티어 상향 검토 표시.
- 네트워크: congested(품질) / quality_ok / unmeasured. 사이징 축이 아니다(vNIC 링크 속도 부재). 재전송·드롭·conntrack 은 품질 신호다.

## 3. 호스트 종합 + 판정 순서

`rollup_host` 가 자원별 판정을 인과 근본원인(root_cause)으로 종합한다. 인과 사슬(상류 -> 하류)은 메모리 ->
디스크 I/O -> CPU 이다. 메모리발은 메모리 포화와 동반 CPU 부족 또는 디스크 I/O 병목으로, 디스크발은 await 병목,
D-state 블록, CPU 포화가 함께 있을 때만 확정한다. 그 외 부족은 독립 원인으로 나열한다.
...
호스트 권고(`recommendation`, 정렬·배지용) 판정 순서 (`_recommendation`, 위에서 먼저 매칭한 순위로 확정):

| 순위 | recommendation | 조건 |
|------|---------------------|------|
...
| 2 | insufficient -> insufficient_data | CPU와 메모리가 모두 unmeasured — 사이징 2축 부재라 disk/network 부분 데이터 있어도 optimal 위장 금지 |
| 3 | idle -> idle | CPU p95, 네트워크 처리량, 디스크 I/O baseline이 모두 유휴 임계 아래(CPU와 네트워크는 측정 필수, 디스크 미측정은 유휴를 막지 않음) |
...
| cpu_saturation | Saturation | `is_cpu_saturated` — 실행 큐/코어 AND 이용률 dual-gate (OS별 신호원 5절) |
| mem_util | Utilization | `assess_memory` — 이용률 p95 단일 임계 |
| mem_saturation | Saturation | `is_memory_saturated` — 이용률 AND 페이징(하드 폴트) dual-gate (OS별 신호원 5절) |
...
| disk_inode | Utilization/추세 | `assess_disk_capacity` — inode runway 임박 OR inode 사용률 정적 가드 |
| disk_io | Saturation | `is_disk_io_saturated` — await p95, device 사용률 게이트를 통과한 버킷만 산출(저활동 device 는 미산출이라 io_ok) |
| net_retrans | Errors(품질) | `assess_network` — 창 전체 delta 합계 비율. 처리량이 10 kB/s 미만이면 비율 판정을 생략하고 quality_ok로 둔다(비율 분모 붕괴 방지) |
| net_drop | Errors(품질) | `assess_network` — 창 전체 delta 합계 비율. 처리량이 10 kB/s 미만이면 판정하지 않는다 |
| net_conntrack | 포화(품질) | `assess_network` — 창 최대 비율, 트래픽 무관 절대 신호 |
| (idle) | Utilization | `_recommendation` — CPU p95, 네트워크 처리량, 디스크 I/O baseline이 모두 유휴 기준을 충족한다. 강도는 `is_strong_idle`(CPU peak + 네트워크)이 즉시 종료 / 통합·재배치 권고로 가름 |
...
메모리는 near-peak 착지(비탄력·OOM 위험이라 피크 대표 통계). 증설·다운사이즈에 같은 통계를 쓴다. under 조건은
충족하지만 목표가 현재 사양을 넘지 않으면, 목표 대신 최소 증설 사양을 표시한다.
...
- cpu_saturation: `is_cpu_saturated` — 실행 큐 신호원은 Linux procs_running/cores, Windows System\Processor Queue Length p95/cores(임계는 OS 별로 다르다). 저활동 실행큐 노이즈(수집기 R-state 자기포함, 특히 1코어)를 이용률 게이트로 차단하고, 이용률 미측정이면 측정된 실행큐를 신뢰한다.
- mem_saturation: `is_memory_saturated` — 페이징 신호원은 Linux paging_major(디스크에서 페이지를 다시 읽는 하드 폴트, `mem_swap_paging` 필드가 싣는 값), Windows Memory Pages Input/sec rate(하드 read 폴트, 총 Pages/sec 과 달리 mmap 미혼입). 스왑 점유량과 pagefile 사용량은 입력이 아니다 — swappiness 로 여유 RAM 에서도 유휴 페이지가 스왑아웃되므로 점유는 압박을 뜻하지 않는다.
- disk_io: `is_disk_io_saturated` — await p95 임계는 양 OS 통일. Windows 도 IOCTL_DISK_PERFORMANCE ReadTime/WriteTime(device 합산 counter_agg)로 await 를 산출한다(Linux time_reading/writing 등가). await를 수집하지 못하면 해당 축은 미관측이다.

Windows 에서 해당 perflib 가 미부착·미발행이면 그 축만 coverage_gap -> `has_unmeasured_saturation`(cpu·mem·disk_io
한정) -> "포화 수치 미관측" 단서다. 분류 자체는 utilization·측정된 나머지 축으로 완결한다.

## 6. 신뢰도 (5종) + 다운사이즈 처방 규칙

신뢰도는 분류와 별개 출력이다 — 측정 불확실성을 종류별로 가른다(`ConfidenceNote`).

- 관측 이력 부족(insufficient_history): 이력이 최소 시간(`CONFIDENCE_MIN_HOURS`) 미만.
- 높은 이용률 변동성(high_utilization_variability): CPU p95/중앙값 비가 임계 초과. 현재 CPU 중앙값만 수집하므로 CPU 평가에만 반영한다.
- 커버리지(coverage_gap): 필요 포화 축 미측정. 측정된 축만으로 분류 + "포화 수치 미관측" 마커.
- 측정 편향(measurement_bias): virtio 오염(steal p95 임계 초과, 게스트 await 하이퍼바이저 간섭) — 표본을 늘려도 해소되지 않는 측정 환경의 왜곡.
- 이용률 상승 추세(rising_utilization_trend): CPU와 메모리의 이용률 상승 추세를 각각 자기 자원 평가에만 반영한다. 호스트 수준 요약이 필요할 때만 둘 중 하나의 상승을 OR로 합친다. forward-looking 결정(다운사이즈·용량 runway)에만 쓴다.

다운사이즈 "처방"은 over 분류가 저사용이면 늘 뜨나, 구체 처방은 신뢰도 높음(정밀·커버리지·충실도 온전) AND 상승
추세 아님 AND 창 관측 충분(`DOWNSIZE_MIN_SAMPLE_COVERAGE`)일 때만 낸다. 관측 비율은 CPU와 메모리 각각의
유효 5분 집계 버킷 수를 `period_days * 288`로 나눈 뒤 작은 값을 쓴다. 미충족이면 과다 표시를 유지하되 권고는
"관찰 지속"이고, right-sizing API의 `recommendation.kind`는 `observe`다.
