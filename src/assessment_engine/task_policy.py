...
task_policy 미보고(bool 발행 안 한 task)면 exit_code + allowlist 폴백으로 판정.
...
(ConsumerSettings.task_install_success_exit_codes) 확장으로 대응 (allowlist 는 task_policy 미보고 폴백 전용).
...
- Linux:   "os_id:major" (예 "rocky:9") — EL9 특정이라 family 키 없음. agent 가 task.result 에
           os_id/os_version 을 직접 발행 (handler 가 그대로 전달).

배경 (둘 다 설치·등록 성공인데 installer 가 non-zero 로 끝나는 false-failure):
- Windows Server 2022(build 20348): installer exit code 2.
- EL9 계열(rocky/almalinux/ol/centos major 9): installer 의 systemd 유닛이 새 systemd
  의 start-limit 에 걸려 exit 3 (설치·등록은 성공). 근본 해결은 installer 유닛 수정, 본 정책은 우회.
...
# task_policy=False 인데 raw 로는 실패 사유가 없는(예: exit 0 success 보고) 경우의 override 사유.
...
def effective_task_result(
    ...
    판정 우선순위 — 실제 설치 신호(task_policy)가 exit_code 추측보다 우선:
      1. task_policy True  -> success. agent 가 데몬 기동+등록을 확인 -> non-zero exit(EL9 exit 3·
         Windows exit 2 같은 false-failure)도 allowlist 없이 정공 흡수.
      2. task_policy False -> failure. exit 0 이어도 데몬 미기동이면 실패 (false positive 차단).
         원래 실패 사유가 있으면 보존, 없으면(success 보고였음) _UNVERIFIED_REASON.
      3. task_policy None(해당 task 가 정책 미보고) -> 아래 exit_code + allowlist 폴백.

    폴백 보정 조건(전부 충족 시에만 success 로 전환):
    ...
    allowlist 는 task_policy 미보고 폴백 전용이다 — 신규 OS 를 allowlist 로 늘리지 않는다.
