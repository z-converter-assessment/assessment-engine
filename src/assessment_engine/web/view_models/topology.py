...
elements: Cytoscape.js elements 형식(`{"data": {...}}` 리스트)으로 mapper 가 precompute.
  ...
  - subnet 노드 data: {id "subnet:<net>", label <net>, kind "subnet"}
  - host 노드 data:   {id "host:<public_id>", label <hostname>, kind "host", publicId, osFamily, multiHomed}
  - edge data:        {source "host:<public_id>", target "subnet:<net>"}
"""
