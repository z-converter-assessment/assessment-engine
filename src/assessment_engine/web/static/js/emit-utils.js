 ...
 * 외부 의존: ToastUtils (선택 — 없으면 토스트 생략).
 */
...
async function submitNavigate(btn, urlFn, opts) {
  ...
  btn.disabled = true;
  const pending = opts.pendingMsg ? ToastUtils.show(opts.pendingMsg, 'pending') : null;
  try {
    ...
    if (!res.ok) {
      ...
      if (ToastUtils) {
        ToastUtils.show((opts.errPrefix || '발행 실패') + ': ' + (msg || '요청을 처리하지 못했습니다.'), 'err');
      }
      btn.disabled = false;
      return;
    }
    ...
    if (pending) pending.remove();
    if (ToastUtils) ToastUtils.show((opts.errPrefix || '발행 요청 실패') + ': ' + (/** @type {Error} */ (e)).message, 'err');
    btn.disabled = false;
  }
}

export { submitNavigate };
