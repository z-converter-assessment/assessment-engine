...
def _extract_attention_hosts(
    ...
    def _slot(public_id: str, hostname: str) -> dict:
        return by_host.setdefault(hostname, {
            "public_id": public_id,
            "hostname": hostname,
            "gap_label": None,
            "os_eol_label": None,
...
def _extract_capacity_imminent(rows: list[ReportRowItem]) -> list[CapacityImminentItem]:
    ...
    for r in rows:
        ...
        if not r.worst_mount:
            continue
        out.append(CapacityImminentItem(
...
def _env_summary_bullets(
    ...
    resource = (
        f"vCPU {overview.total_vcpus} / 메모리 {overview.total_memory_gb:.1f} GB"
        f" / 디스크 {overview.total_disk_gb} GB"
    )
    distribution = (
        f"under {under} / over {over} / idle {idle}"
        f" / optimal {optimal} / insufficient {insufficient}"
    )
    bullets = [
...
def to_environment_report(
    *,
    ...
    under_hosts = (
        ...
        if under_provisioned_hosts is not None
        else list(overview.under_provisioned_hosts)
    )
