...
from assessment_engine.worker.settings import worker_settings
...
async def main() -> None:
    setup_logging(worker_settings.log_format)
    ...
    report_task = asyncio.create_task(
        run_report_worker(
            ...
            poll_interval_sec=worker_settings.report_worker_poll_interval_sec,
            stale_seconds=worker_settings.report_worker_stale_seconds,
    ...
    reaper_task = asyncio.create_task(
        run_task_reaper(
            ...
            interval_sec=worker_settings.install_reaper_interval_sec,
            stop_event=stop_event,
        )
    )
    try:
        ...
        await graceful_drain(
            report_task, stop_event, worker_settings.report_worker_shutdown_timeout_sec,
            "report worker shutdown timeout — in-flight job left running (stale recovery will requeue)",
        )
        await graceful_drain(
            reaper_task, stop_event, worker_settings.install_reaper_shutdown_timeout_sec,
