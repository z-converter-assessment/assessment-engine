...
전제는 셋이다 — Docker(compose plugin 포함), [uv](https://docs.astral.sh/uv/), Node. Python 은 uv 가 준비하고, [pnpm](https://pnpm.io/) 은 `corepack enable` 로 `package.json` 의 `packageManager` 핀을 따라 붙는다.
...
기동 전제·첫 기동 소요·초기화는 `docs/guides/local-dev.md`.
...
로컬 커밋은 lint 만 건다. 코드 리뷰·테스트·문서·ADR 은 develop PR 게이트가 본다 — 이벤트별 게이트 배치와 그 근거는 `docs/guides/pre-pr-checklist.md` 0절.
...
문서는 목적으로 나뉜다 — 고치기 전에 그 사실이 어느 목적에 속하는지, 이미 다른 문서에 있지는 않은지를 `docs/README.md` 에서 확인한다. 관리 규칙 전체가 거기 있다.
